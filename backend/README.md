...
```
  Сообщение от пользователей, которые не являются администраторами, игнорируется. При получении сообщения от
  администратора, оно пересылается всем клиентам в комнате. Пользователи не являющиеся администраторами, отключаются от
  комнаты. Подключиться к закрытой комнате могут только администраторы. В комнате можно создать пользователя только по
  админской ссылке.
...
```
  Сообщение от пользователей, которые не являются администраторами, игнорируется. При получении сообщения от
  администратора, оно пересылается всем клиентам в комнате.
